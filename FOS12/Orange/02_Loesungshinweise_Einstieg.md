...
> Tipp: Lassen Sie die SuS die Achsenwahl ausprobieren. Mit `sepal length`/`sepal width` ist die Überlappung noch stärker. `petal length`/`petal width` bzw. `sepal length`/`petal width` zeigt die klarste Trennung.
...
3. **Ab welchem Wert korrekte Trennung:** Ab Polynomial expansion 2–3 verbessert sich die Trennung deutlich; die Klassen sind dann in der Regel getrennt (abhängig von der Variablenwahl).
